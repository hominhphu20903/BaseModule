...
abstract class ColorSlider(context: Context, attributeSet: AttributeSet?) :
    ...
    var strokeSize = context.dpToPx(2)
        set(value) {
            field = value
            invalidate()
        }

    private var defaultPaddingVertical = context.dpToPx(12)
    private var wrapContentSize = context.dpToPx(48).toInt()

    init {
        context.theme.obtainStyledAttributes(attributeSet, R.styleable.ColorPickerView, 0, 0).apply {
            try {
                lineStrokeCap = Paint.Cap.values()[getInt(
                    R.styleable.ColorPickerView_sliderBarStrokeCap,
                    Paint.Cap.ROUND.ordinal
                )]

                strokeSize = getDimension(R.styleable.ColorPickerView_sliderStrokeSize, strokeSize)

                strokeColor = getColor(R.styleable.ColorPickerView_sliderStrokeColor, strokeColor)
