...
class ColorHueSlider(context: Context, attributeSet: AttributeSet?) :
    ColorSlider(context, attributeSet) {

    constructor(context: Context) : this(context, null)

    private var hsvHolder = FloatArray(3)
    private var hsvCurrentHolder = FloatArray(3)
    ...
    override fun calculateBounds(targetWidth: Float, targetHeight: Float) {
        ...
        circleX = drawingStart + (hsvHolder[0] / 360f) * (widthF - drawingStart) // Custom
        circleColor = calculateColorAt(circleX)
    }
    ...
    fun setInitialColor(color: Int) {
        ...
        Color.colorToHSV(color, hsvCurrentHolder)
        invalidate()
    }
}